...
class FileUploadRequest(BaseModel):
    filename: str
    content: str
    encoding: str = 'utf8'  # 'utf8' for CSV, 'base64' for ZIP/binary
...
async def upload_layer_data(
    ...
    # Decode content — CSV arrives as UTF-8 text, ZIP arrives as base64
    ...
    if filename.endswith('.csv'):
        ...
        raise HTTPException(status_code=400, detail="Only CSV (.csv) and Shapefile ZIP (.zip) files are supported")

    if feature_count == 0:
        raise HTTPException(status_code=400, detail="No valid features found. Check file format and coordinate columns.")
